...
func_install_cdr-port () { 
				...
				mysql -u root -p"$DB_PASSWORD" cdrport < $CONFIG_DIR/install/sql/ramal.sql
				mysql -u root -p"$DB_PASSWORD" cdrport < $CONFIG_DIR/install/sql/tmp_canais.sql
...
func_install_req_asterisk () { 
				...
				libssl-dev libnewt-dev libusb-dev libeditline-dev libedit-dev libssl-dev libmysqlclient-dev libjansson-dev
				ExitFinish=1
}
